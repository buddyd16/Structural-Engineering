...
class elastic_weld_group:
    ...
    def force_analysis(self, Fz, Fx, Fy, Mx, My, Mz):
        ...
        #Resultant Force
        self.f1 = math.sqrt(((fz+mx+my)*(fz+mx+my)) + ((fx-mzx)*(fx-mzx)) + ((fy-mzy)*(fy-mzy)))
        self.log = self.log + '\nf1 = [(fz+mx+my)^2 + (fx-mzx)^2 + (fy-mzy)^2]^1/2= {0:.3f}'.format(self.f1)
        self.f2 = math.sqrt(((fz+mx-my)*(fz+mx-my)) + ((fx-mzx)*(fx-mzx)) + ((fy+mzy)*(fy+mzy)))
